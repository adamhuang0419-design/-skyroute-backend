...
app.get('/api/places', async (req, res) => {
  ...
  try {
    const resp = await fetch('https://api.duffel.com/places/suggestions?query=' + encodeURIComponent(q), {
      headers: { 'Accept-Encoding': 'gzip', 'Duffel-Version': 'v2', 'Authorization': 'Bearer ' + key }
    });
    const json = await resp.json();
    if (!resp.ok) return res.json([]);
    const items = (json.data || [])
      .filter(function (p) { return p.iata_code; })
      .slice(0, 8)
      .map(function (p) {
        return { iataCode: p.iata_code, name: p.name, type: p.type, cityName: (p.city && p.city.name) || p.name };
      });
    res.json(items);
  } catch (e) {
